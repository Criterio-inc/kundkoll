...
extension Tester {
    static func mötesserier() {
        ...
        func möte(_ titel: String, _ dag: String) -> Möte {
            Möte(inspelning: Inspelning(titel: titel, inledd: Uppgift.dag(dag)!, längd: 60, kund: "Acme",
                        projekt: nil, mikrofon: nil, liveYttranden: [], arkivYttranden: nil),
                 mapp: URL(fileURLWithPath: "/x/\(titel)"))
        }
        ...
        Prov.lika(Mötesserie.föregående(b.inspelning, bland: alla)?.inspelning.id, a.inspelning.id,
                  "närmast föregående i serien hittas")
        Prov.lika(Mötesserie.föregående(a.inspelning, bland: alla)?.inspelning.id, d.inspelning.id,
                  "och kedjan fortsätter bakåt")
        Prov.lika(Mötesserie.föregående(d.inspelning, bland: alla)?.inspelning.id, nil,
                  "det första mötet har inget före sig")
        Prov.lika(Mötesserie.föregående(c.inspelning, bland: alla)?.inspelning.id, nil,
                  "ett möte utanför serien hör inte hit")

        let siffror = möte("20260901", "2026-09-01")
        Prov.lika(Mötesserie.föregående(siffror.inspelning, bland: alla + [möte("20260801", "2026-08-01")])?.inspelning.id,
